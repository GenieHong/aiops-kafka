...
menu_option_10() {
  echo "Read Message Specific Topic"
  oc get kafkatopic -n $WAIOPS_NAMESPACE
  read -p "Copy Paste Topic from above: " MY_TOPIC

  ${KAFKACAT_EXE} -v -X security.protocol=SASL_SSL -X ssl.ca.location=./ca.crt -X sasl.mechanisms=SCRAM-SHA-512 -X sasl.username=$SASL_USER -X sasl.password=$SASL_PASSWORD -b $BROKER \
    -C -t $MY_TOPIC \
    # -o beginning
    # -o-3 \
    # -c3 \
...
menu_option_11() {
  echo "Publish Message Specific Topic"
  oc get kafkatopic -n $WAIOPS_NAMESPACE
  read -p "Copy Paste Topic from above: " MY_TOPIC
...
if [ -x "$(command -v kafkacat)" ]; then
  ...
  export SASL_USER=$(oc get secret ibm-cp-watson-aiops-kafka-secret  -n $WAIOPS_NAMESPACE --template={{.data.username}} | base64 --decode)
  export SASL_PASSWORD=$(oc get secret ibm-cp-watson-aiops-kafka-secret  -n $WAIOPS_NAMESPACE --template={{.data.password}} | base64 --decode)
...
until [ "$selection" = "0" ]; do
  ...
  echo "        10  - Read Message Specific Topic"
  echo "        11  - Publish Message Specific Topic"
  echo "      "
  echo "        0  -  Exit"
  echo ""
  echo ""
